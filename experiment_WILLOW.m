...
incrementCnt = 15;
scrResult = zeros(4,incrementCnt);
accResult = zeros(4, incrementCnt);
conResult = zeros(4, incrementCnt);
timeResult = zeros(4, incrementCnt);
matchResult = zeros(incrementCnt);
...
test_i = 0;
savePath = fullfile(resDir, class, sprintf('in%02d_out%02d_%02d.mat', inCnt, outCnt, test_i));
...
for i = 1:incrementCnt
    ...
    accResult(1,i)=mean(accIncreCAO(:));
    scrResult(1,i)=mean(scrIncreCAO(:));
    conResult(1,i)=mean(conIncreCAO(:));
    timeResult(1,i)=tEnd;
    ...
    accResult(2,i)=mean(accRawCAO(:));
    scrResult(2,i)=mean(scrRawCAO(:));
    conResult(2,i)=mean(conRawCAO(:));
    timeResult(2,i)=tEnd;
    ...
    tStart = tic;
    [imgmIncreMatching, numPairMatch] = IMGM_single_step(affinity, simAP, imgmMatTmp, param);
    tEnd = toc(tStart);
...
save(savePath, 'accResult', 'timeResult', 'scrResult', 'conResult', 'matchResult');
