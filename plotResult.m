function newFig = plotResult(legendOff, cate_config, ave, algSet, xtag, ytag, displayOrder)
    ...
    if cate_config(end)<=cate_config(1)
        ...
        % if cate_config(end) - cate_config(1) > 15
    ...
    if y1<1 && y2<=1.5
        ...
        for yk = 1:length(ytickNum)
            ytickText{yk} = num2str(ytickNum(yk),'%.2f');
        end
