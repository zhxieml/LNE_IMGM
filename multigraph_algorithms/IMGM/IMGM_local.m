...
function M = IMGM_local(affinity, affScore, rawMat, target, param)
    [a,b]=size(affScore);
    if a~=b
...
%     end
    ...
    if param.method == isDPP
        ...
        [idx, partition] = dpp_graph_partition(affScoreTmp,2);
        C_index = unique(idx);
        idx(indMaxCon) = -1;
        %toc
    end
    ...
    if param.method == isRand
        %tic;
        nCluster = 2; % number of the clusters
    ...
    for i=1:length(C_index)
        ...
        %%%%%%%%%%%%%%%% crop the affinity %%%%%%%%%%%%%%%%%%%%%
        affinityCrop = crop_affinity(tmpClusterPosition, affinity);
        %%%%%%%%%%%%%%%% crop the target %%%%%%%%%%%%%%%%%%%%%%%
        targetCrop = crop_target(tmpClusterPosition, target);
