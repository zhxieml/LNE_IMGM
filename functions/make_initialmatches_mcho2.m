...
for i=1:maxFeatType
    ...
    if mparam.bReflective
        [ tmpInitialMatch, simdot ] = descmatch_dot( viewInfo(view1).desc(:,range1),...
            viewInfo(view2).desc_ref(:,range2),bSelf , mparam.distRatio, mparam.distThres, mparam.kNN, mparam.fullMatch) ;
    else
        %try
        [ tmpInitialMatch, simdot ] = descmatch_dot( viewInfo(view1).desc(:,range1),...
            viewInfo(view2).desc(:,range2), bSelf , mparam.distRatio, mparam.distThres, mparam.kNN, mparam.fullMatch) ;
        %catch
        %    pause;
        %end
    end
