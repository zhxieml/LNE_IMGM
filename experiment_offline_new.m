...
graphMaxCnt = 20;
graphMinCnt = 5;
graphCntRange = graphMinCnt:5:graphMaxCnt;
% graphStep = 1;
...
for parak = 1:paraCnt
    ...
    for testk = 1:testCnt
        ...
        if algSet.algEnable(tbimgm_cao_cIdx)
            ...
            tStart = tic;
            [Matching{tbimgm_cao_cIdx}, numPairMatch] = TBIMGM_offline(rawMat,nodeCnt,graphCnt,param);
            tEnd = toc(tStart);
        ...
        if algSet.algEnable(tbimgm_cao_c_adaIdx)
            ...
            tStart = tic;
            [Matching{tbimgm_cao_c_adaIdx}, numPairMatch] = TBIMGM_offline(rawMat,nodeCnt,graphCnt,param);
            tEnd = toc(tStart);
            ...
            accAve(parak, tbimgm_cao_c_adaIdx, testk) = mean(acc{tbimgm_cao_c_adaIdx}(:));
            scrAve(parak, tbimgm_cao_c_adaIdx, testk) = mean(scr{tbimgm_cao_c_adaIdx}(:));
            conPairAve(parak, tbimgm_cao_c_adaIdx, testk) = mean(con{tbimgm_cao_c_adaIdx}(:));
            timAve(parak, tbimgm_cao_c_adaIdx, testk) = tEnd;
            countPairAve(parak, tbimgm_cao_c_adaIdx, testk) = numPairMatch;
        end
